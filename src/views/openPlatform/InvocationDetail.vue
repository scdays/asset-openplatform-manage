    ...
    <a-spin :spinning="loading">
      <div v-if="detail" class="detail-header">
        ...
        <div class="detail-actions">
          ...
          <a-button v-if="detail.hasRequestBody" @click="openRequestBody">查看请求报文</a-button>
          <a-button v-if="detail.hasRequestBody" @click="downloadRequestBody">下载请求 TXT</a-button>
      ...
      <div v-if="detail" class="detail-grid">
        ...
        <div class="detail-main">
          <a-card title="请求头 / 响应头" :bordered="false" class="detail-card">
            <div class="payload-grid">
              <div class="payload-box">
                <div class="payload-label">Request Headers <a class="copy-link" @click="copyText(detail.requestHeadersPreview, '请求头')">复制</a></div>
                <pre class="code-block">{{ detail.requestHeadersPreview }}</pre>
              </div>
              <div class="payload-box">
                <div class="payload-label">Response Headers <a class="copy-link" @click="copyText(detail.responseHeadersPreview, '响应头')">复制</a></div>
            ...
            <div class="payload-grid">
              <div class="payload-box">
                ...
                <div v-if="detail.hasRequestBody" class="response-body-actions">
                  <p class="helper">
                    请求报文较大，默认不加载。
                    <span v-if="detail.requestBodyByteSize">已持久化约 {{ formatBytes(detail.requestBodyByteSize) }}。</span>
                  </p>
          ...
          <a-card v-if="detail.responseCode !== 0 && detail.errorMessage" title="错误详情" :bordered="false" class="detail-card">
...
export default {
  ...
  methods: {
    ...
    copyText (text, label) {
      if (!text) { this.$message.warning('暂无可复制内容'); return }
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(() => { this.$message.success(label + ' 已复制') }).catch(() => this.fallbackCopy(text, label))
