<template>
  ...
  >
    <template slot="extra">
      <span class="drawer-hint">快速预览调用摘要，完整报文请打开详情页</span>
    </template>
      ...
      <div v-if="detail">
        <a-tabs v-model="activeTab">
          ...
          <a-tab-pane key="payload" tab="报文/响应">
            <p class="helper">Request Body 为摘要；Response Body 按需二次加载。</p>
          ...
          <a-tab-pane key="resource" tab="关联资源">
            ...
            <div v-if="detail.relatedWebhooks && detail.relatedWebhooks.length" class="related-webhooks">
              ...
              <a-table
                ...
                :data-source="detail.relatedWebhooks"
              >
...
export default {
  ...
  components: { EnumTag, ResponseCodeTag, InvocationResponseBodyModal },
  props: {
    visible: {
      type: Boolean,
      default: false
  ...
  computed: {
    drawerTitle () {
      if (this.detail && this.detail.operationId) {
        return `调用预览 · ${this.detail.operationId}`
      }
      return '调用预览 · 加载中...'
    }
  },
  watch: {
    visible (val) {
      if (val) {
        ...
        this.detail = null
        this.responseBodyVisible = false
      }
    },
    invocationId () {
      if (this.visible) {
  ...
  methods: {
    ...
    openFullPage () {
      if (!this.invocationId) return
      this.$emit('close')
      this.$router.push({ name: 'InvocationDetail', params: { invocationId: this.invocationId } })
    },
    handleClose () {
      this.$emit('close')
    },
